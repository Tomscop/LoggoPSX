	...
	//Fallthrough
		...
		case MenuPage_Story:
		{
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (pad_state.press & (PAD_START | PAD_CROSS))
				{
					...
					if ((menu_options[menu.select].stage == StageId_1_1) && (menu.page_param.stage.diff == StageDiff_Hard))
						menu.page_param.stage.id = StageId_1_2;
		...
		case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_1_1, 0xFF9271FD, "CHRISTMAS"},
				{StageId_1_3, 0xFF9271FD, "SPOOKPOSTOR"},
				{StageId_1_4, 0xFF9271FD, "GRINCH"},
			};
